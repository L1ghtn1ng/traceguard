...
const (
	maxDomainFileBytes   = 8 << 20
	maxDomainFileEntries = 32768
)
...
func loadDomainFile(path string) ([]string, error) {
	...
	payload, err := safefile.ReadFile(path, maxDomainFileBytes)
	if err != nil {
		return nil, fmt.Errorf("read domain file %q: %w", path, err)
	}
	return parseDomainEntries(bytes.NewReader(payload))
}
...
func parseDomainEntries(r io.Reader) ([]string, error) {
	...
	for scanner.Scan() {
		...
		lineEntries := splitDomainEntries(line)
		if len(lineEntries) > maxDomainFileEntries-len(entries) {
			return nil, fmt.Errorf("domain entry count exceeds %d", maxDomainFileEntries)
		}
