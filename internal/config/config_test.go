...
func TestParseRejectsUnexpectedPositionalArgs(t *testing.T) {
	...
	os.Args = []string{"traceguard", "README.md", "LICENSE"}
	...
	if !strings.Contains(err.Error(), "unexpected positional arguments") {
		t.Fatalf("Parse error = %q, want positional argument rejection", err)
	}
}
...
func TestParseRejectsRemovedLegacyPolicyFlags(t *testing.T) {
	for _, name := range []string{"blocklist-url", "block-domain", "allow-domain", "cache-path", "refresh-interval"} {
		t.Run(name, func(t *testing.T) {
			originalArgs := os.Args
			t.Cleanup(func() { os.Args = originalArgs })
			os.Args = []string{"traceguard", "-" + name, "value"}
			clearPolicyEnv(t)

			_, err := Parse()
			if err == nil || !strings.Contains(err.Error(), "flag provided but not defined") {
				t.Fatalf("Parse error = %v, want removed flag rejection", err)
			}
		})
	}
}

func TestParseAcceptsYAMLPolicyAsBlockingSource(t *testing.T) {
	originalArgs := os.Args
	t.Cleanup(func() { os.Args = originalArgs })
	os.Args = []string{
		"traceguard",
		...
		"-policy-url", "https://policy.example/v1",
		"-policy-client-cert", "/etc/traceguard/client.crt",
		"-policy-client-key", "/etc/traceguard/client.key",
	}
	clearPolicyEnv(t)
	...
	if got, want := cfg.PolicyPath, "/etc/traceguard/policy.yaml"; got != want {
		t.Fatalf("PolicyPath = %q, want %q", got, want)
	}
	if got, want := cfg.PolicyURL, "https://policy.example/v1"; got != want {
		t.Fatalf("PolicyURL = %q, want %q", got, want)
	}
}

func TestParseRejectsMalformedYAMLPolicyConfiguration(t *testing.T) {
	for name, args := range map[string][]string{
		"insecure URL":  {"traceguard", "-policy-url", "http://policy.example/v1"},
		"relative path": {"traceguard", "-policy-path", "policy.yaml"},
...
func TestParseIgnoresRemovedLegacyPolicyEnvironment(t *testing.T) {
	t.Setenv("TRACEGUARD_BLOCK", "true")
	t.Setenv("TRACEGUARD_BLOCK_ALL", "true")
	t.Setenv("TRACEGUARD_BLOCKLIST_URL", "https://legacy.example/blocklist.txt")
	t.Setenv("TRACEGUARD_BLOCK_DOMAINS", "*")
	t.Setenv("TRACEGUARD_ALLOW_DOMAINS", "example.com")
	t.Setenv("TRACEGUARD_DRY_RUN", "")
	t.Setenv("TRACEGUARD_POLICY_PATH", "")
	t.Setenv("TRACEGUARD_POLICY_URL", "")
	...
	if err == nil || !strings.Contains(err.Error(), "require at least one policy source") {
		t.Fatalf("Parse error = %v, want missing policy source after ignoring removed policy environment variables", err)
	}
}
